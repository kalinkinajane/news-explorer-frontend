...
//         const timeStamp = date.setDate(date.getDate() - 7);
//         const newtimStamp = new Date(timeStamp);
//         const dateFrom = newtimStamp.getFullYear()+'-'+("0"+(newtimStamp.getMonth()+1)).slice(-2)+'-'+("0"+newtimStamp.getDate()).slice(-2);
//         const dateNow = new Date().getFullYear()+'-'+("0"+(new Date().getMonth()+1)).slice(-2)+'-'+("0"+new Date().getDate()).slice(-2);
//         return fetch(`${this.baseUrl}/everything?language=ru&q=${keyword}&from=${dateFrom}&to=${dateNow}&pageSize=100&apiKey=${this.apiKey}`,{
//             method: "GET",
//             headers: {
